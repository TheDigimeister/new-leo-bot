...
void lemlib::Chassis::moveDistance(float distance, int timeout, MoveDistanceParams params, bool async){
    params.earlyExitRange = fabs(params.earlyExitRange);
    this->requestMotionStart();
    // were all motions cancelled?
    if(!this->motionRunning) return;
    //if the function is async, run it back in a new task
    if(async){
    ...
    //reset PID and exit conditions
    lateralPID.reset();
    lateralLargeExit.reset();
    lateralSmallExit.reset();
    angularPID.reset();

    // initialize vars used bettwen iterations
    ...
    const float target_theta = getPose(true,true).theta;
    float target_x = distance * cos(target_theta) + lastPose.x;
    float target_y = distance * sin(target_theta) + lastPose.y;
    Pose target(target_x,target_y,target_theta);
    ...
    while (!timer.isDone() && ((!lateralSmallExit.getExit() && !lateralLargeExit.getExit()) || !close) && this->motionRunning) {
        ...
        const float distTarget = params.forwards ? (distance - distTraveled) : -(distance - distTraveled);
        //check if the robot is close enough to the target to start settling
        if (fabs(distTarget) < 7.5 && close == false){
            close = true;
        ...
        // calculate error
        // const float adjustedRobotTheta = params.forwards ? pose.theta + M_PI : pose.theta;
        const float angularError = params.straight ? angleError(pose.theta, target_theta,true) : 0;
